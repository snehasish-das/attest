...
$app->get('/users', function (Request $request, Response $response, array $args) {
    $getUser = "SELECT `user_id`, `name`, `email`, `phone`, `dob`, `address`, `city`, `zip`, `image`, `role`, `company`, `website`, `whatsapp`, `is_phone_verified`, `is_email_verified` FROM `users` where is_deleted=0 ";
...
//Get
$app->get('/users/verify', function (Request $request, Response $response) {
    $user_id = $_SESSION['user_id'];
    try {
        $db = new db();
        $db = $db->connect();

        $user = $db->query("SELECT `name` as user_name, `email` as user_email, `is_phone_verified`, `is_email_verified` FROM users WHERE `user_id`='$user_id'")->fetch(PDO::FETCH_ASSOC);
...
//Get
$app->get('/users/login', function (Request $request, Response $response, array $args) {
    $user_id = $_SESSION['user_id'];
    try {
        $db = new db();
        $db = $db->connect();
        $user = $db->query("SELECT `user_id`, `name`, `email`, `phone`, `dob`, `address`, `city`, `zip`, `image`, `role`, `company`, `website`, `whatsapp`, `is_phone_verified`, `is_email_verified` FROM users WHERE `user_id`='$user_id'")->fetch(PDO::FETCH_ASSOC);
        if (sizeof($user) > 0) {
            //@$user['Auth'] = base64_encode($user['email'].":".$user['password']);
            return $response->withStatus(200)->write(json_encode($user));
        }
        else{
...
$app->post('/users', function (Request $request, Response $response) {
    ...
    try {
        ...
        $user = $db->query("SELECT `user_id`, `name`, `email`, `phone`, `dob`, `address`, `city`, `zip`, `image`, `role`, `company`, `website`, `whatsapp`, `is_phone_verified`, `is_email_verified` FROM users WHERE phone='$phone'")->fetch(PDO::FETCH_ASSOC);
        if(@$user['phone'] != ''){
            return $response->withStatus(400)->write('{"error" : {"text": "Phone is already registered" } }');
        }

        $user = $db->query("SELECT `user_id`, `name`, `email`, `phone`, `dob`, `address`, `city`, `zip`, `image`, `role`, `company`, `website`, `whatsapp`, `is_phone_verified`, `is_email_verified` FROM users WHERE email='$email'")->fetch(PDO::FETCH_ASSOC);
        if(@$user['email'] != ''){
            return $response->withStatus(400)->write('{"error" : {"text": "Email is already registered" } }');
        }

        $addQuery = "INSERT INTO `users` (`user_id`, `name`, `email`, `phone`, `password`, `dob`, `address`, `city`, `zip`, `image`, `role`, `company`, `website`, `whatsapp`, `is_phone_verified`, `is_email_verified`) VALUES ((SELECT UUID()), :name, :email, :phone, :password, :dob, :address, :city, :zip, :image, :role, :company, :website, :whatsapp, :is_phone_verified, :is_email_verified)";
        ...
        if ($stmt->execute()) {
            $user = $db->query("SELECT `user_id`, `name` as `user_name`, `email` as `user_email`, `phone`, `dob`, `address`, `city`, `zip`, `image`, `role`, `company`, `website`, `whatsapp`, `is_phone_verified`, `is_email_verified` FROM users WHERE email='$email'")->fetch(PDO::FETCH_ASSOC);
            $user['register_otp'] = substr(rand(),0,6);
...
$app->patch('/users', function (Request $request, Response $response) {
    ...
    try {
        ...
        if ($stmt->execute()) {
            $user = $db->query("SELECT `user_id`, `name`, `email`, `phone`, `dob`, `address`, `city`, `zip`, `image`, `role`, `company`, `website`, `whatsapp`, `is_phone_verified`, `is_email_verified` FROM users WHERE `user_id`='$id'")->fetch(PDO::FETCH_ASSOC);
            return $response->withStatus(200)->write(json_encode($user));
        } else {
...
$app->delete('/users', function (Request $request, Response $response) {
    $id = $_SESSION['user_id'];
    $deleteQuery = "UPDATE `users` SET `is_deleted`=1 WHERE `user_id`='$id'";
